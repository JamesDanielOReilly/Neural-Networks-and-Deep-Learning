...
elseif strcmp(cmd,'leftup') & ~isempty(fig)
  ...
  if nnaxclik(left,track_data{1},track_data{2})
    ...
    axes(right)
    if number >= 1
      b = cell2mat(trans_data([1 8; 2 9]));
      c = cell2mat(trans_data([4 11; 5 12]));
      a = c/b;
...
elseif strcmp(cmd,'clear') & ~isempty(fig)
  ...
  for i=7:7:numel(trans_data)
    set(trans_data{i-4},'color',ltyell)
    set(trans_data{i-1},'color',ltyell)
    set(trans_data{i},'color',ltyell)
    delete([trans_data{[i-4 i-1 i]}]);
  end
